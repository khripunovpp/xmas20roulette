...
adc.phone = {
  ...
  init: function () {
    var $phone = $(this.setting.phone),
        $country = $(this.setting.country),
        countryCode = $country.val().toLowerCase();
    if(this.db[countryCode]) $phone.mask(this.db[countryCode]);
    $country.on('change', this.db, function (e) {
      countryCode = $(this).val();
      var countryCodeLowerCase = countryCode.toLowerCase();
      $phone.val('');
      if(e.data[countryCodeLowerCase]) {
        $country.each(function () {
          $(this).find('[value=' + countryCode + ']').prop('selected', true);
        });
        $phone.mask(e.data[countryCodeLowerCase]);
      }else{
        $phone.unmask();
      }
    });
